...
def check_existing_preprocessed_files(label="", required_files=None):
    ...
    if not os.path.exists(output_folder):
        ...
        if all_files_exist:
            logger.info(f"All required files for the selected threshold, 'filtered_common_labs', 'unimputed_vector', 'feature_matrix' and 'imputed_vector' are found in '{output_folder}' folder. Therefore, we are going to use already stored files for further processing.\n")
            return True
        else:
...
def save_preprocessing_outputs(filtered_common_labs_percent, pivot_percent, feature_matrix_df, imputed_preprocessed, label=""):
    ensure_folder_exists()
...
# Construct the Feature matrix (2D NumPy array) from the wide-form pivoted data, excluding identifier columns.
def get_feature_matrix(pivot):
    ...
    '''
    logger.info(
        "Constructing the Feature matrix(it is 2D NumPy array where each row corresponds to an admission (hadm_id) and each column to a specific lab–day measurement) prior to the Normalization and Imputation, will serve as the basis for custom pairwise distance calculations to compute different types of Clustering using Distance matrix.\n"
    )
